...
Drop your unorganized files into the staging folder, open the Import page, and match them to albums or tracks. Commissary takes care of the rest: full metadata tagging (artist, album, track number, genres), album art embedding, lyrics fetching, renaming to your path template, and moving everything into your organized library. Files go from a chaotic pile to properly tagged, organized tracks in your transfer folder.
...
Then set the staging path in Commissary settings to `/app/Staging`.
...
You can organize files in subfolders — Commissary reads folder names as hints for album suggestions (e.g., a folder named `Artist - Album` improves matching).
...
Commissary automatically suggests albums based on your files' metadata tags and folder structure. These appear as album cards with cover art.
...
Commissary matches your staging files to album tracks using title similarity and track numbering. Each match shows a confidence percentage:
...
You can also click **"Re-match Automatically"** to reset all manual overrides and let Commissary re-run its matching.
...
For each matched track, Commissary:
