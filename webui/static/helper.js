...
const WHATS_NEW = {
    ...
    '1.6.6': [
        { date: 'July 2026 · 1.6.6' },
        { title: 'Downloads land in the right Library', desc: 'grabbing an episode of a show filed under Anime saved it into the standard TV folder. The per-episode buttons sent no Library at all, and the Get dropdown pre-picked the first one and sent that explicitly — so an Anime show was handed the TV category. Every download path now resolves the Library the title actually lives in.' },
        { title: 'Anime and other long titles stop being rejected', desc: 'a release carrying a work\'s full name — "…Zero Subjects: Tales of Blue Dias and the Onikin Alna" — was refused as a wrong title, because TMDB stores only the part before the colon. Releases are now also matched on that shorter form, with guards so Dune: Part Two still can\'t satisfy a search for Dune.' },
        { title: 'New titles remember their Library', desc: 'the wishlist only knew a Library through a title you already owned, so a brand-new Anime show had none and always downloaded into the primary TV Library. It now carries its own.' },
        { title: 'Saving Libraries no longer drops your YouTube folder', desc: 'the settings page saves Movies and TV, and that quietly deleted every YouTube Library row — removing it from health checks, the recycle bin and moved-file resolution until it was re-added.' },
        { title: 'Library filters match the tab you are on', desc: 'the genre and quality dropdowns listed every value across all Libraries of a kind, so on the Anime tab you could pick a genre that returned nothing.' },
        { title: 'Earlier versions', desc: '1.6.5 stopped Library tabs reading as duplicates (All Movies / All TV / All Shows), gave each per-Library tab a count, and fixed Place this file never finding shows. 1.6.4 added Preferred Groups and per-Library preferred trackers, moved the Wishlist/Download History Library filters into the tab strips (they had rendered blank and only worked for admins), gave the dashboard Library widget one tile per Library with its own disk usage, and fixed the app version having been stuck at 1.6.1 since 1.6.2 — which had also been silently hiding the release notes. 1.6.3 added manual import for any on-disk file, fixed fansub-style anime releases being rejected as title mismatches, and root-caused dashboard customisation being dead on the Video side. 1.6.2 made unattended grabs respect each title\'s own Library. 1.6.1 fixed torrent/usenet downloads stuck at 100% when the release was a single bare file. 1.6.0 folded in 38 fixes from upstream SoulSync (through 3.1.8), led by a security fix for indexer URLs leaking to the browser. 1.5.0 removed the Soulseek chat feature and the Support button, and gave Plex a stable device identity. 1.4.0 let every user drag and resize their own dashboard cards, and accepted more than one completed-downloads folder. 1.3.2 made your video Libraries drive the health checks, recycle bin, moved-file resolver and naming-repair job. 1.3.1 added collapsible albums in Purchased. 1.3.0 extended the standard-user policy to the sidebar. 1.2.0 let admins choose which dashboard cards standard users see. 1.1.0 brought the Purchased page. 1.0.0 was this fork\'s baseline, carrying upstream SoulSync 3.1.5.' },
    ],
};
...
const VERSION_MODAL_SECTIONS = [
    {
        title: "1.6.6: downloads go where the title actually lives",
