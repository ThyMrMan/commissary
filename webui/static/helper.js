...
const WHATS_NEW = {
    ...
    '1.6.5': [
        { date: 'July 2026 · 1.6.5' },
        { title: 'No more duplicate-looking Library tabs', desc: 'a Library called "Movies" sitting under a tab called "Movies" read as the same entry twice, as did "TV" beside "TV Shows". The kind tab now says it is the union — All Movies, All TV, All Shows — whenever its own Libraries are listed next to it. Applies to the Wishlist, Download History and the Enrichment Workers strip.' },
        { title: 'Library tabs show how much is in them', desc: 'each per-Library tab now carries a count, like the Movies/TV tabs always have. A show Library counts distinct shows rather than episodes, so its number is directly comparable to the All TV total above it.' },
        { title: 'Place this file finds shows again', desc: 'searching for a series in the manual-import dialog only ever returned it under the Movie tab — the picker was reading fields the search endpoint does not send, so every result silently fell back to "movie". Shows now appear under Episode where they belong.' },
        { title: 'Earlier versions', desc: '1.6.4 added Preferred Groups and per-Library preferred trackers, moved the Wishlist/Download History Library filters into the tab strips (they had rendered blank and only worked for admins), gave the dashboard Library widget one tile per Library with its own disk usage, taught manual import to recognise episodes from the filename, and fixed the app version having been stuck at 1.6.1 since 1.6.2 — which had also been silently hiding the release notes. 1.6.3 added manual import for any on-disk file, fixed fansub-style anime releases being rejected as title mismatches, and root-caused dashboard customisation being dead on the Video side. 1.6.2 made unattended grabs respect each title\'s own Library. 1.6.1 fixed torrent/usenet downloads stuck at 100% when the release was a single bare file. 1.6.0 folded in 38 fixes from upstream SoulSync (through 3.1.8), led by a security fix for indexer URLs leaking to the browser. 1.5.0 removed the Soulseek chat feature and the Support button, and gave Plex a stable device identity. 1.4.0 let every user drag and resize their own dashboard cards, and accepted more than one completed-downloads folder. 1.3.2 made your video Libraries drive the health checks, recycle bin, moved-file resolver and naming-repair job. 1.3.1 added collapsible albums in Purchased. 1.3.0 extended the standard-user policy to the sidebar. 1.2.0 let admins choose which dashboard cards standard users see. 1.1.0 brought the Purchased page. 1.0.0 was this fork\'s baseline, carrying upstream SoulSync 3.1.5.' },
    ],
};
...
const VERSION_MODAL_SECTIONS = [
    {
        title: "1.6.5: the Library tabs read properly now",
