...
const WHATS_NEW = {
    ...
    '2.3.9': [
        { date: 'September 2026 · 2.3.9' },
        { title: "Tracks discovery can't match now come with suggestions", desc: "when no candidate was good enough, playlist discovery made a Wing It guess and threw away everything else it had found, so fixing the track meant searching again from scratch. It now keeps the best few near misses with the track — saved with a mirrored playlist, and kept by the Playlist Pipeline too. A row shows its top suggestion with an Accept button, “✓ Accept all” takes every suggestion at or above the “Accept suggestions ≥” percentage you set, the Fix dialog lists them straight away, and the Wing It Pool offers the same." },
        { title: "Mirrored Qobuz playlists match tracks exactly by ISRC", desc: "every Qobuz track carries an ISRC — the code every release of one recording shares — but it never got past the Qobuz tab, so discovery matched by title and artist alone. The Qobuz tab now stores it when it mirrors a playlist (playlists it has already mirrored pick it up the next time the tab loads them), and discovering a mirrored playlist, by hand or through the Playlist Pipeline, looks the code up on Deezer before any search, whichever metadata source you use — iTunes has no ISRC lookup. A match is used only when its length agrees to within five seconds and Deezer gives its album and its place on it." },
        { title: "The Fix dialog searches every source at once", desc: "it used to search your main metadata source and try the next only when nothing came back. It now asks every source together and lists their answers as they arrive, each with its artwork, its source and how far its length is from the playlist's track. Chips filter by source, and karaoke, cover, live and remix versions sink below the rest or hide. A pick from a source other than your main one is completed from its own source first — track and disc number, album, release date — so a download no longer tags it from whatever your main source keeps under the same number. Fix Match in the Discovery Pool and the Wing It Pool searches every source too, so it works without a Spotify connection." },
        { title: "Review a playlist's unmatched tracks from the keyboard", desc: "🎯 Review in a discovery window steps through every track still waiting for a decision — unmatched tracks, Wing It guesses, errors and low-confidence matches — in one dialog. 1–9 choose a result, Enter accepts it, S skips and Esc closes; a pick is saved at once and the next track loads, with no Confirm click. “Not available” marks a track no source has: its match or guess is cleared so nothing syncs or downloads it, and Review, Retry Failed, the Playlist Pipeline and later discoveries of a mirrored playlist leave it alone until you fix it or put it back with ↺." },
        { title: "Fixes and removed matches are kept", desc: "a Fix on a YouTube or mirrored playlist was remembered under the picked song's title with no artist — a name no discovery ever looks for — so the next discovery of that song searched all over again. Remove match on a mirrored playlist was never saved, so the match was back the next time the playlist opened. And Rematch in the Discovery Pool saved the new match where discovery never looks for it. All three now stick, and a removed match stays Not Found even after you change metadata source." },
        { title: "Mirrored playlists discovered from their own tab no longer shift a row", desc: "a Qobuz, Deezer or Beatport playlist discovered from its tab saved each result onto the mirrored track before it: the first result was lost and every other one landed a row early. Mirrored Wing It guesses also reopened as “Provider changed” with nothing in them, and a playlist of nothing but guesses reopened as never discovered." },
        { title: "Counts and buttons that fell behind", desc: "a Fix on a Tidal, Deezer, Spotify-link, Apple Music-link or Beatport playlist left its card's match count behind, and the filter chips kept their old counts after any change. Fix on a playlist opened from the Qobuz tab, or on an Apple Music link, only said “Track data not found”, and Remove match there went to YouTube's route; Remove match couldn't find ListenBrainz playlists at all. Removing or fixing a Wing It guess could miscount as well: Tidal, Deezer, Qobuz, link and Beatport playlists count a guess as a match and YouTube, mirrored and ListenBrainz playlists don't, and every change now follows the playlist's own rule." },
        { title: "iTunes searches no longer wait twice", desc: "iTunes allows about 20 requests a minute, so searches are spaced three seconds apart — and every new search waited for that twice, while even a search the cache could answer waited before looking. Discovering a playlist with iTunes cost around seven seconds for each track it hadn't seen. A search now waits once, and only before a real request." },
        { title: "…and what still doesn't", desc: "Retry Failed on a mirrored playlist searches a track whose match you removed, and the cache can hand that same match straight back. On the Tidal, Deezer, Qobuz, link and Beatport tabs, Remove match and Not available change only the open results, and a mirrored copy of the playlist keeps its match. A playlist discovered from the Qobuz tab itself still matches by title and artist — only mirrored playlists use ISRCs. Tracks discovered before this update have no suggestions until they're discovered again." },
    ],
    '2.3.8': [
...
const VERSION_MODAL_SECTIONS = [
    {
        title: "2.3.9: discovery suggestions, exact ISRC matches, and a Fix dialog that searches everywhere",
        description: "Discovery keeps its near misses to accept, matches mirrored Qobuz tracks by ISRC, and fixes a playlist from one keyboard-driven Review dialog that asks every metadata source at once - plus fixes, removals and counts that were quietly lost.",
        features: [
            "core/discovery/suggestions: a track below the auto-match bar keeps its best few candidates, shaped like Fix search rows, saved in a mirrored track's extra_data and cleared once a match is picked",
            "discovery rows offer the top suggestion; Accept all takes every one at or above the modal's 'Accept suggestions' percentage; the Fix dialog and the Wing It Pool list them",
            "core/discovery/isrc_match: a mirrored track with an ISRC resolves on Deezer (get_track_by_isrc) before the cache or any search - within 5 s, and only with its album's track count, release date and its track position",
            "the Qobuz client keeps each track's ISRC (core/text/isrc normalizes it), the Qobuz tab's mirror saves it, and an ISRC match is exempt from provider-drift redo",
            "GET /api/discovery/fix-search asks each source in its own request; answers merge as they land, ranked by relevance, with artwork, source, length delta and source chips; karaoke, cover, live and remix versions sink or hide",
            "core/discovery/fix_pick completes an iTunes or Deezer pick from its own source (track and disc number, album id, release date, track count) before any Fix route saves it",
            "Review steps through every row that isn't Perfect or Not available: 1-9 choose, Enter accepts, S skips, Esc closes; POST /api/discovery/unavailable marks a track Not available and discovery passes skip it",
            "YouTube and mirrored Fix routes cache the pick under the source track's own key (they used the picked title and no artist); Discovery Pool Rematch replaces the entry under its own key and provider",
            "mirrored write-back maps result index i to mirrored position i + 1 and reads Qobuz and Deezer source-track ids, not only Tidal's; a mirrored Wing It guess reopens as a guess, not 'Provider changed'",
            "unmatch saves a mirrored track's cleared match with unmatched_by_user, has a Qobuz route and finds ListenBrainz states by profile; guess counts follow each platform (_GUESSES_COUNT_AS_MATCHES, discoveryGuessesCount)",
            "card counts and filter chips follow a Fix, Remove match or Not available; Fix Match in the Discovery Pool and the Wing It Pool asks every source",
            "iTunes search_tracks and search_albums rate-limit once, and a cached answer doesn't wait",
        ],
        usage_note: "Nothing to change. Tracks discovered before this update have no suggestions until they're discovered again. Retry Failed on a mirrored playlist can still bring back a match you removed, and on the source tabs Remove match and Not available last only while the results are open.",
    },
    {
