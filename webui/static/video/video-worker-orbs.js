...
// enrichment buttons, uses its own window.videoWorkerOrbs global, and is
// activated by the video side's page events. The music orbs file is never
// touched and the two instances never share state. Same exact animation.
//
// Video dashboard header buttons shrink to floating orbs, expand on hover.
// ============================================================
...
(function () {
    ...
    // Status pushes arrive ~every 2.5s. Spread each window's pulses across that
    // interval so they drip steadily instead of bursting on arrival.
    ...
    function init() {
        const sub = document.querySelector('.video-subpage[data-video-subpage="video-dashboard"]');
        dashboardHeader = sub ? sub.querySelector('.dashboard-header') : null;
        headerActions = sub ? sub.querySelector('.header-actions') : null;
        ...
        WORKER_DEFS.forEach((def, i) => {
            const found = headerActions.querySelector(def.sel);
            if (!found) return;
            // Orb's positioned element = the enrich container (so hiding it tucks
            // the button + tooltip away); the hub button has no container.
            const el = found.closest('.video-enrich-container') || found;
            const btn = found.matches('button') ? found : found.querySelector('button');
            ...
            orbs.push({
                ...
                active: false,
                statusSeen: false,    // has a real status arrived for this worker?
    ...
    function tick() {
        ...
        for (const orb of visibleOrbs) {
            ...
            if (orb.statusSeen) {
                // Release queued pulses at a steady drip so a window of events
                // streams up the spoke instead of arriving all at once.
    ...
    // ── Page awareness (video side) ──

    function isEnabled() {
        return window._videoWorkerOrbsEnabled !== false && !window._reduceEffectsActive;
    }

    function onVideoDashboard() {
        return document.body.getAttribute('data-side') === 'video'
            && document.body.getAttribute('data-video-page') === 'video-dashboard'
            && isEnabled();
    }
    ...
    function refresh() {
        if (onVideoDashboard()) activate();
        else deactivate();
    }

    // ── Real telemetry → pulses ──
    // Fed by video-enrichment.js's status poll. We diff the cumulative counters
    // between updates and queue one inbound pulse per real item processed
    // (brand colour) or error (red). No status yet → the loop falls back to an
    // ambient trickle so active orbs still animate.
    function onStatus(id, data) {
        ...
        const processed = num(s.matched) + num(s.not_found);
        ...
        if (dErr > 0) {
            ...
            // stress up gradually. Small bump so it ramps in softly, never spikes.
    ...
    function bootstrap() {
        ...
        window.videoWorkerOrbs = { refresh, onStatus };

        // The video side announces page changes; switching back to the music
        // side flips body[data-side]. Watch both so the orbs activate only on
        // the video dashboard and tear down everywhere else.
        document.addEventListener('soulsync:video-page-shown', refresh);
        const mo = new MutationObserver(refresh);
        mo.observe(document.body, { attributes: true, attributeFilter: ['data-side', 'data-video-page'] });

        // Catch the case where we boot already on the video dashboard.
