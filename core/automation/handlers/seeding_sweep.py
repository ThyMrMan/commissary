"""Automation handler: ``seeding_sweep`` — the music torrent seed-until-goals tail.

Thin progress wrapper around :mod:`core.downloads.seeding` (which owns the
overlap guard, the goal judgment and the client removal). Music-side twin of
``video_seeding_sweep``."""
...
logger = get_logger("automation.seeding_sweep")


def auto_seeding_sweep(config: Dict[str, Any], deps: AutomationDeps) -> Dict[str, Any]:
    from core.downloads.seeding import sweep
    ...
    if res.get("status") == "skipped":
        msg = {"already_running": "Previous sweep still running — skipped",
               "no_goals_set": "No seed goals set (Settings → Downloads) — skipping",
               "no_torrent_client": "No torrent client configured — skipping"}.get(
